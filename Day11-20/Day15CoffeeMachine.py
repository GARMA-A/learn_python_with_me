...
def askAndReturnDolars():
    ...
    DolarsAmount = round(
        float(
            (quarterNumber * 25 + dimeNumber * 10 + nickelNumber * 5 + pennyNumber)
            / 100
        ),
        2,
    )
...
machineResourses = {
    ...
    "milk": 200,
    "coffee": 100,
    "money": round(float(0.00), 2),
    "curPayOperation": round(float(0.00), 2),
}
...
while not Exit:
    ...
    if coffeeType == "nop!":
        Exit = (
            input(
                "\nYou must Enter a Type ! type 'y' to contuniue or 'n' to Exit  : "
            ).lower()
            == "n"
        )
        ...
        Exit = (
            input("\nDo You Want To Exit The Programm ? type 'y' or 'n' : ").lower()
            == "y"
        )
        ...
        complete = (
            input(
                "\nespresso Price is 1.5$ do you want to contuniue ? type 'y' or 'n'  : "
            ).lower()
            == "n"
        )
        ...
        while not complete:
            ...
            if espressoRequirements["money"] - machineResourses["curPayOperation"] == 0:
                ...
                Exit = (
                    input(
                        "\nDo You Want To Exit The Programm ? type 'y' or 'n' : "
                    ).lower()
                    == "y"
                )
                ...
                espressoRequirements["money"] - machineResourses["curPayOperation"] > 0
            ):

                complete = (
                    input(
                        f"\nyou still need to pay {round( float(espressoRequirements['money'] -  machineResourses['curPayOperation']) , 2)}\
              to complete  'complete' , 'dont complete' : "
                    )
                    != "complete"
                )
                ...
                machineResourses["water"] -= espressoRequirements["water"]
                machineResourses["coffee"] -= espressoRequirements["coffee"]
                machineResourses["money"] -= round(
                    machineResourses["curPayOperation"] - espressoRequirements["money"],
                    2,
                )
        ...
        coffeeType == "latte"
        and machineResourses["milk"] >= latteRequirements["milk"]
        and machineResourses["coffee"] >= latteRequirements["coffee"]
        and machineResourses["water"] >= latteRequirements["water"]
    ):
        complete = (
            input(
                "\nlatte Price is 2.5$ do you want to contuniue ? type 'y' or 'n'  : "
            ).lower()
            == "n"
        )
        ...
        while not complete:
            ...
            if latteRequirements["money"] - machineResourses["curPayOperation"] == 0:
              ...
              to complete  'complete' , 'dont complete' : "
                ...
                machineResourses["milk"] -= latteRequirements["milk"]
                machineResourses["money"] -= round(
                    machineResourses["curPayOperation"] - latteRequirements["money"], 2
                )
                machineResourses["money"] += machineResourses["curPayOperation"]
                machineResourses["curPayOperation"] = 0
        ...
        coffeeType == "latte"
        and machineResourses["milk"] < latteRequirements["milk"]
        or machineResourses["coffee"] < latteRequirements["coffee"]
        ...
        coffeeType == "capucino"
        and machineResourses["milk"] >= capucionRequirements["milk"]
        and machineResourses["coffee"] >= capucionRequirements["coffee"]
        and machineResourses["water"] >= capucionRequirements["water"]
    ):
        complete = (
            input(
                "\nlatte Price is 3.0$ do you want to contuniue ? type 'y' or 'n'  : "
            ).lower()
            == "n"
        )
        ...
        while not complete:
            ...
            if capucionRequirements["money"] - machineResourses["curPayOperation"] == 0:
                ...
                capucionRequirements["money"] - machineResourses["curPayOperation"] > 0
            ):

                complete = (
                    input(
                        f"you still need to pay {round( float(capucionRequirements['money'] -  machineResourses['curPayOperation']) , 2)}\
              to complete  'complete' , 'dont complete' : "
                    ).lower()
                    != "complete"
                )
                ...
                print(
                    f"Done ! you will receive rest of your money { round( machineResourses['curPayOperation']-capucionRequirements['money'],2)}$.\n\
                  your coffee will be ready in a minute... "
                )
                ...
                machineResourses["milk"] -= capucionRequirements["milk"]
                machineResourses["money"] -= round(
                    machineResourses["curPayOperation"] - capucionRequirements["money"],
                    2,
                )
                machineResourses["money"] += machineResourses["curPayOperation"]
                machineResourses["curPayOperation"] = 0
        ...
        coffeeType == "capucino"
        and machineResourses["milk"] < capucionRequirements["milk"]
        or machineResourses["coffee"] < capucionRequirements["coffee"]
        or machineResourses["water"] < capucionRequirements["water"]
    ):

        print("Not enough resourses in the machine try again soon !")
        Exit = (
            input("Do You Want To Exit The Programm ? type 'y' or 'n' : ").lower()
            == "y"
        )
